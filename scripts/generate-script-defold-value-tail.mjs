...
function validateCrossInputProvenance(ir, patterns, accounting, irText, patternsText) {
  ...
  assert(accounting.schemaVersion === 1 && typeof accounting.defoldRevision === "string" && Array.isArray(accounting.rows) && accounting.inputEvidence && typeof accounting.inputEvidence === "object", "script accounting schema is unsupported");
  assert(patterns.defoldRevision === ir.defoldRevision && accounting.defoldRevision === ir.defoldRevision, "script IR, patterns, and accounting Defold revisions differ");
  assert(patterns.sourceSha256 === sha256(irText), "script binding patterns are stale against script IR");
  assert(accounting.inputEvidence.scriptIrSha256 === sha256(irText), "script accounting is stale against script IR");
  assert(accounting.inputEvidence.bindingPatternsSha256 === sha256(patternsText), "script accounting is stale against script binding patterns");
  ...
  const accountingIds = identities(accounting.rows, "script accounting rows");
  sameIdentitySet(accountingIds, irIds, "script accounting rows", "script IR functions");
  const classifiedAccountingIds = new Set(accounting.rows.filter(({ category }) => category !== "separate-module").map(({ id }) => id));
  sameIdentitySet(patternIds, classifiedAccountingIds, "script binding patterns", "classified script accounting rows");
  ...
  assert(accounting.functionCount === ir.functions.length, "script accounting function count differs from script IR");
}
...
export function generateScriptDefoldValueTail(inputs) {
  ...
  const pending = new Set(accounting.rows.filter(({ category, reason }) =>
    category === "pending" && reason?.loweringFamily === "defold-value").map(({ id }) => id));
  const tailPatterns = patternRows.filter(({ id }) => pending.has(id));
  assert(Number.isInteger(accounting.pendingByLoweringFamily?.["defold-value"]) && accounting.pendingByLoweringFamily["defold-value"] === pending.size,
    "script accounting defold-value pending count drifted");
  ...
  for (const family of policy.families) {
    ...
    for (const group of family.sourceRoutes) {
      ...
      for (const id of group.ids) {
        ...
        const entry = { id, stableId: stableBindingId(id), modulePath: fn.modulePath, member: fn.member, sourcePath: source.path, disposition: family.disposition, family: family.id, backend: family.backend ?? null };
  ...
  const report = {
    ...
    coverageClaim: "Candidate routes have deterministic, source-pinned descriptors and exact ABI shape/result validation only. They are not executable until a captured-Lua adapter that preserves the listed Defold value codecs and active engine context is installed. Blocked routes have no generated adapter.",
    allocationClaim: "Descriptor lookup and validation use static sorted tables and caller-owned ScriptCallFrame storage. The future captured-Lua adapter must cache its Lua function references and use fixed-capacity stack/scratch; this generator makes no allocation or packaged-engine-execution claim.",
    targetSupport: { nativeDynamicHermes: "candidate-metadata-and-fail-closed-dispatch-only", nativeStaticHermes: "not-integrated", html5BrowserHost: "not-executable-no-provider" },
    inputEvidence: { scriptIrSha256: sha256(inputs.irText), bindingPatternsSha256: sha256(inputs.patternsText), accountingSha256: sha256(inputs.accountingText), reviewedPolicySha256: sha256(inputs.policyText), defoldSources: policy.sources.map(({ path, sha256: hash }) => ({ path: `upstream/defold/${path}`, sha256: hash })).sort((a, b) => compare(a.path, b.path)) },
...
export async function loadScriptDefoldValueTailInputs() {
  const [irText, patternsText, accountingText, policyText] = await Promise.all([readFile(paths.ir, "utf8"), readFile(paths.patterns, "utf8"), readFile(paths.accounting, "utf8"), readFile(paths.policy, "utf8")]);
  ...
  return { irText, patternsText, accountingText, policyText, sourceTexts };
}

function renderTarget(report) {
  const routes = report.bindings.map(({ id, stableId, disposition, family, resultCodec, blocker }) => ({ id, stableId: `0x${stableId.toString(16).padStart(8, "0")}`, disposition, family, resultCodec, blocker }));
