...
async function readInputs(root = repositoryRoot) {
  const [projectionText, callbacksText, classificationText, policyText, layoutsText] = await Promise.all([
  ...
  return { projectionText, callbacksText, classificationText, policyText, layoutsText };
}
...
function renderSource(rows) {
  const operations = rows.map((row) =>
    `  {0x${row.stableId.toString(16).padStart(8, "0")}u, ${quote(row.id)}, ${quote(row.modulePath.join("."))}, ${quote(row.member)}, ${row.minimumArgumentCount}, ${row.maximumArgumentCount}, ${row.minimumResultCount}, ${row.maximumResultCount}, ${row.maximumResultCount}, ${row.resultSemanticKindId}, ${row.frameContract.inputEntryCapacity}, ${row.frameContract.outputEntryCapacity}, ${row.frameContract.matrix4Arena ? 1 : 0}, ${row.frameContract.urlArena ? 1 : 0}},`
  ).join("\n");
  return `// Generated by scripts/generate-script-universal-value-bindings.mjs. Do not edit.
...
export function renderStaticHermes(layouts, options = {}) {
  const elements = matrix4Elements(layouts);
  const lanes = urlLanes(layouts);
  const matrix4Kind = 4;
  const urlHandleKind = 2;
  ...
  const exact = options.verification === true
    ? {
        base: " exactTag():number{return -1;} exactNumber(slot:number):number{return 0;} exactString():string{return \"\";}",
    ...
    : { base: "", undefined: "", null: "", boolean: "", number: "", string: "", handle: "", defold: "", matrix4: "", url: "" };
  return `// Generated by scripts/generate-script-universal-value-bindings.mjs. Do not edit.
...
class DehermStaticValue { encode(frame:c_ptr,ancestors:Array<DehermStaticContainer>,depth:number):number{throw "deherm Static Hermes abstract value";} asString():string{throw "deherm Static Hermes value is not a string";} probeSize():number{return 0;} probeChecksum():number{return 0;}${exact.base} }
class DehermStaticUndefined extends DehermStaticValue { encode(frame:c_ptr,ancestors:Array<DehermStaticContainer>,depth:number):number{return __pushUndefined(frame);}${exact.undefined} }
class DehermStaticNull extends DehermStaticValue { encode(frame:c_ptr,ancestors:Array<DehermStaticContainer>,depth:number):number{return __pushNull(frame);}${exact.null} }
class DehermStaticBoolean extends DehermStaticValue { value:boolean; constructor(value:boolean){super();this.value=value;} encode(frame:c_ptr,ancestors:Array<DehermStaticContainer>,depth:number):number{return __pushBoolean(frame,this.value?1:0);}${exact.boolean} }
class DehermStaticNumber extends DehermStaticValue { value:number; constructor(value:number){super();this.value=value;} encode(frame:c_ptr,ancestors:Array<DehermStaticContainer>,depth:number):number{return __pushNumber(frame,this.value);}${exact.number} }
class DehermStaticString extends DehermStaticValue { value:string; constructor(value:string){super();this.value=value;} encode(frame:c_ptr,ancestors:Array<DehermStaticContainer>,depth:number):number{return __writeUtf8(frame,this.value);} asString():string{return this.value;}${exact.string} }
class DehermStaticHandle extends DehermStaticValue { kind:number; semanticKind:number; runtime:number; payloadLow:number; payloadHigh:number; released:boolean; constructor(kind:number,semanticKind:number,runtime:number,payloadLow:number,payloadHigh:number){super();this.kind=kind;this.semanticKind=semanticKind;this.runtime=runtime;this.payloadLow=payloadLow;this.payloadHigh=payloadHigh;this.released=false;} encode(frame:c_ptr,ancestors:Array<DehermStaticContainer>,depth:number):number{return __pushHandle(frame,this.kind,this.semanticKind,this.runtime,this.payloadLow,this.payloadHigh);} probeChecksum():number{return this.payloadLow+this.payloadHigh;}${exact.handle} dispose():void{if(!this.released){this.released=true;__releaseHandle(this.kind,this.runtime,this.payloadLow,this.payloadHigh);}} }
class DehermStaticDefoldValue extends DehermStaticValue { kind:number; x:number;y:number;z:number;w:number; constructor(kind:number,x:number,y:number,z:number,w:number){super();this.kind=kind;this.x=x;this.y=y;this.z=z;this.w=w;} encode(frame:c_ptr,ancestors:Array<DehermStaticContainer>,depth:number):number{return __pushDefold(frame,this.kind,this.x,this.y,this.z,this.w);} probeChecksum():number{return this.x+this.y+this.z+this.w;}${exact.defold} }
...
class DehermStaticUrl extends DehermStaticValue { ${urlParameters.map((name) => `${name}:number;`).join(" ")} constructor(${urlParameters.map((name) => `${name}:number`).join(",")}){super();${urlParameters.map((name) => `this.${name}=${name};`).join("")}} encode(frame:c_ptr,ancestors:Array<DehermStaticContainer>,depth:number):number{let index=__pushUrl(frame,${urlParameters.map((name) => `this.${name}`).join(",")});if(index===0xffffffff)throw "deherm Static Hermes URL arena exhausted";return index;} probeSize():number{return ${lanes.length};} probeChecksum():number{return ${urlParameters.map((name) => `this.${name}`).join("+")};}${exact.url} }
