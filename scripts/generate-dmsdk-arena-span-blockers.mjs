...
export function generate(inputs) {
  ...
  // Derived inputs must agree with each other; the reviewed policy is compared
  // against the revision being generated by the reviewed-revision rule. The
  // policy's substance - its tranche, its prior-wave report list and their IR
  // and ABI-shape provenance hashes - is re-checked below at that revision.
  const defoldRevision = ir.defoldRevision;
  assert(shapes.defoldRevision === defoldRevision, "arena-span blocker inputs use different Defold revisions");
  assertReviewedRevision({
    input: paths.policy,
    reviewed: policy.defoldRevision,
    derived: defoldRevision,
    detail: "the reviewed arena/span blocker tranche"
  });
  ...
  for (const expected of policy.priorWaveReports) {
    ...
    assert(report.defoldRevision === defoldRevision, `${expected.path}: Defold revision drifted`);
