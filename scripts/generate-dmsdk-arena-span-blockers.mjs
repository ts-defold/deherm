...
function renderExactFake(entry) {
  ...
  return `dmURI::Result ${name}(const char* input,char* output,uint32_t capacity,uint32_t* written){${observe}${write}if(written)*written=UINT32_C(${`result_${id}`.length + 1});return dmURI::RESULT_OK;}`;
}
...
export async function loadInputs(root = repositoryRoot) {
  const [irText, shapesText, policyText] = await Promise.all([readFile(resolve(root, paths.ir), "utf8"), readFile(resolve(root, paths.shapes), "utf8"), readFile(resolve(root, paths.policy), "utf8")]);
  ...
  return { irText, shapesText, policyText, priorWaveTexts, evidenceTexts };
}
export function generate(inputs) {
  ...
  const entries = selected.map((candidate, bindingId) => {
    ...
    return { bindingId, candidate, recipe };
  });
  ...
  const generatedDeclarations = entries.map(({ bindingId, candidate, recipe }) => ({ ...candidate, disposition: "generated", preferredLowering: false, denseId: bindingId, wrapper: "deherm_dmsdk_arena_cstring_dispatch", exactWrapper: "deherm_dmsdk_arena_cstring_exact_dispatch", exactCallee: exactCallee({ bindingId }), recipe: { kind: recipe.kind, shape: recipe.shape }, universalFallback: "retained-usage-materialized-recipe", sourceEvidence: recipe.sourceEvidence, stages: { generated: "production-and-exact-from-one-recipe", compiled: "pinned-header-production-and-exact-object-tests", linked: "exact-recording-callee", runtime: "exact-dispatch-all-vectors", allocation: "bounded-thread-local-input-scratch-no-heap-primitives" } }));
  const sourceHashes = { ir: sha256(inputs.irText), shapes: sha256(inputs.shapesText), policy: sha256(inputs.policyText), priorWaveReports: Object.fromEntries(priorWaves.map(({ report, sha256: hash }) => [report, hash])), evidence: Object.fromEntries([...inputs.evidenceTexts].sort(([a], [b]) => compare(a, b)).map(([path, text]) => [path, sha256(text)])) };
