...
export async function run(argv = process.argv.slice(2)) {
  ...
  const temporary = path.join(options.outRoot, `.dmsdk-universal-check-${process.pid}`);
  const report = await buildUniversalDmSdkBindings({ outRoot: temporary, projectionPath: options.projection, sdkIrPath: options.sdkIr });
  try {
