...
function validatePolicyCoverage(selected, policy) {
  ...
  if (JSON.stringify(selectedIds) !== JSON.stringify(policyIds)) {
    throw new Error(`hash-span policy must account for exactly ${selected.length} mechanically selected candidates`);
  }
}
...
async function createEntries(selected, declarations, policy, sources) {
  ...
  for (const candidate of selected) {
    ...
    const rule = policy.entries[candidate.id];
    if (!declaration || rule.status !== "emit" || ![32, 64].includes(rule.resultBits)
...
function createReport(contents, ir, shapes, policy, sources, entries, selected, artifacts) {
  return {
    schemaVersion: 1,
...
async function build(options) {
  ...
  const entries = await createEntries(selected, declarations, policy, sources);
