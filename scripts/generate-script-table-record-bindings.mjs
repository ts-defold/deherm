...
export function generate(inputs) {
  ...
  const rows = policy.routes.map((rule) => {
    ...
    assert(pendingIds.has(rule.id), `${rule.id}: reviewed table-record route is not accounting-pending lua-table`);
  ...
  const blockedRoutes = pending.filter(({ id }) => !selectedIds.has(id)).map(({ id }) => { const schema = schemaById.get(id); return { id, stableId: stableBindingId(id), bucket: schema.bucket, blocker: blockerFor(schema) }; }).sort((left, right) => left.stableId - right.stableId || compare(left.id, right.id));
