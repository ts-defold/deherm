...
const scalarTypes = new Map([["boolean", "Boolean"], ["integer", "Integer"], ["number", "Number"], ["string", "String"]]);
const scalarArgumentTypes = new Map([["boolean", "Boolean"], ["integer", "Integer"], ["number", "Number"], ["string", "String"]]);
...
function fieldCodec(field, id) {
  ...
  assert(!field.optional, `${id}: first-wave fixed-record field '${field.rawName}' is optional`);
  return codec;
}
...
function routeIsMechanicallySafe(schema, fn, types) {
  ...
  const resultAlternatives = fn.returns.length === 1 ? fn.returns[0].split("|").map((value) => value.trim()) : [];
  if (resultAlternatives.length < 1 || resultAlternatives.length > 2 ||
      (resultAlternatives.length === 2 && !resultAlternatives.includes("nil"))) return false;
  const recordType = resultAlternatives.find((value) => value !== "nil");
  if (!recordType || tableResults[0].rawType !== fn.returns[0]) return false;
  if (fn.parameters.some(({ rawType }) => !scalarArgumentTypes.has(rawType))) return false;
...
function renderRuntime(rows) {
  const fields = rows.flatMap(({ fields }) => fields);
  const args = rows.flatMap(({ argumentCodecs }) => argumentCodecs);
  const fieldOffsets = []; let offset = 0;
  for (const row of rows) { fieldOffsets.push(offset); offset += row.fields.length; }
  const argOffsets = []; offset = 0;
  for (const row of rows) { argOffsets.push(offset); offset += row.argumentCodecs.length; }
...
export function generate(inputs) {
  ...
  const pending = accounting.rows.filter(({ category, id }) => category === "pending" && patternById.get(id)?.loweringFamily === "lua-table");
  assert(pending.length === policy.expectedPendingLuaTableRouteCount, `table-record pending lua-table census drifted: ${pending.length}`);
  assert(new Set(pending.map(({ id }) => id)).size === pending.length, "table-record pending lua-table identities are duplicated");
  const pendingIds = new Set(pending.map(({ id }) => id));
  assert(schemas.familyCounts?.["lua-table"] === pending.length, "table-record schema lua-table census drifted");
  for (const { id } of pending) {
    const schema = schemaById.get(id), pattern = patternById.get(id);
    assert(schema?.family === "lua-table" && pattern && JSON.stringify(schema.parameters) === JSON.stringify(pattern.parameterCodecs) && JSON.stringify(schema.returns) === JSON.stringify(pattern.returnCodecs), `${id}: table-record schema classification drifted`);
  }
  const safeIds = new Set(pending.map(({ id }) => id).filter((id) => routeIsMechanicallySafe(schemaById.get(id), fnById.get(id), types)));
  const seen = new Set(); const rows = policy.routes.map((rule) => {
    assert(!seen.has(rule.id), `${rule.id}: duplicate reviewed table-record route`); seen.add(rule.id);
    assert(pendingIds.has(rule.id), `${rule.id}: reviewed table-record route is not accounting-pending lua-table`); assert(safeIds.has(rule.id), `${rule.id}: reviewed route is not a mechanically safe finite fixed record`);
    ...
    const resultAlternatives = fn.returns.length === 1 ? fn.returns[0].split("|").map((value) => value.trim()) : [];
    assert(type?.kind === "class" && fn.returns.length === 1 && resultAlternatives.includes(rule.recordType) && schema.returns.length === 1 && schema.returns[0].rawType === fn.returns[0], `${rule.id}: reviewed record type drifted`);
    const argumentCodecs = fn.parameters.map(({ rawType }) => scalarArgumentTypes.get(rawType)); assert(argumentCodecs.every(Boolean), `${rule.id}: reviewed scalar argument type drifted`);
    return { id: rule.id, stableId: stableBindingId(rule.id), modulePath: fn.modulePath, member: fn.member, source: `upstream/defold/${source.path}`, sourceSha256: source.sha256, sourceAnchors: rule.sourceAnchors, argumentCodecs, fields: type.fields.map((field) => ({ name: field.rawName, codec: fieldCodec(field, rule.id) })), reason: rule.reason };
  ...
  const blockedRoutes = pending.filter(({ id }) => !selectedIds.has(id)).map(({ id }) => {
    const schema = schemaById.get(id); return { id, stableId: stableBindingId(id), bucket: schema.bucket, blocker: blockerFor(schema) };
  }).sort((left, right) => left.stableId - right.stableId || compare(left.id, right.id));
...
if (process.argv[1] && pathToFileURL(process.argv[1]).href === import.meta.url) run(process.argv.includes("--check")).then((report) => console.log(`${process.argv.includes("--check") ? "Verified" : "Generated"} ${report.candidateCount}/${report.routeCount} fixed-record Lua-table candidates; ${report.executableCount} executable.`)).catch((error) => { console.error(error.stack ?? error.message); process.exitCode = 1; });
