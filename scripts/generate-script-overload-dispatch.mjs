...
const urls = {
  ...
  override: new URL("bindings/overrides/script-overload-dispatch.json", root),
  owned: new URL("bindings/generated/defold-script-value-bindings.json", root),
  report: new URL("bindings/generated/defold-script-overload-dispatch.json", root),
...
export function generate(inputs) {
  ...
  const rows = selected.map((pattern) => {
    ...
    return { id: fn.id, stableId: stableBindingId(fn.id), modulePath: fn.modulePath, member: fn.member,
      ...
      targetSupport: candidate ? { nativeDynamicHermes: "candidate-awaits-shared-router-integration", nativeStaticHermes: "planned-generated-adapter", html5BrowserHost: "not-executable-no-generated-provider" } :
  ...
  const report = { schemaVersion: 1, defoldRevision: ir.defoldRevision,
    ...
    evidencePolicy: "Pinned source hashes and anchors record the reviewed registration surface. The generated native dispatcher only validates the finite Defold-value call shapes and delegates exact Lua execution to an injected backend. It is not connected to the shared ScriptAdapter/JSI router, so candidates are not executable-accounting promotions and no engine or browser conformance is claimed.",
    allocationPolicy: "Dispatch performs a sorted static descriptor lookup and scans only the reviewed fixed call shapes. It allocates no heap storage and fails closed before invoking a backend when an argument or result does not match a reviewed shape.",
