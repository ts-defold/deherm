...
| Static Hermes | Sound-TypeScript constraints, the C ABI, and the pinned Defold value layouts are modeled separately | Static application unit loading, vmath bridge, a generated fixed-capacity universal value frame with Matrix4/URL arenas, a typed-native bridge, a generator-owned canonical 517-route script exact unit, and a generated four-deep thread-local dmSDK exact-call frame exist | Strict typed recursive scalar/string/array/record materialization, transparent Defold value records, and borrowed GUI-node handles compile and run; the canonical typed-native exact set replays 517/517 routes through the production bounded frame. Test-only exact accessors are emitted only into the build-directory verification transport, not the committed production classes | `shermes -emit-c` output links into focused native exact executables; the project-owned extension output is uploaded by Bob, compiled and linked by the pinned local Extender into an arm64-macOS engine, and evaluated into the same Hermes runtime as the bytecode bundle | The canonical typed-native exact set reports 517 planned/executed with zero mismatches. Its recursive argument/result checks consume interned exact values, generation fails closed on lane, driven-arity, bounds, arenas, shapes, and release drift, and the executable has repeatable ASan/UBSan coverage. The dmSDK gate remains separate. War Battles' project-owned gate also observes compilation, Bob/Extender linkage, and engine launch with typed-native unit registration; this is transport evidence rather than Defold implementation semantics or full-game Static AOT gameplay | Both value and dmSDK transports use bounded reentrant pools. The 517-route census is exact transport evidence and does not claim per-route negative exhaustion beyond its focused lifecycle fixtures, every dynamic/overload alternative, optional-result omission, or allocator instrumentation. Reachability and applicability remain distinct from verification; full War Battles gameplay remains Dynamic Hermes | Preserve exact coverage while adding real-engine semantic evidence; retain handle/callback policy as shapes begin emitting and keep full-game Static AOT explicitly unproven |
...
plan select all 27 of the project's 27 routes for Static Hermes, with zero
reachable blockers. The canonical typed-native exact set is 517/517, and the
project-owned bridge source is pinned by SHA-256
...
compilation and Bob/Extender linkage, plus an engine launch that observed the
typed-native unit registration marker. It is a generated adapter and transport
projection, not a claim that the full War Battles application has executed as
Static Hermes AOT; full gameplay remains Dynamic Hermes and Defold semantics
remain separate gates. The fifth
