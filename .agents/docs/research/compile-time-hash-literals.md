...
| Development bundler | War Battles GUI compiles through `createIncrementalCompiler` and `@ttsc/unplugin/esbuild` | Nine input-action hashes are constants; final bundle contains no `hashLiteral`, `defold.hash`, or hash stable-ID dispatch |
