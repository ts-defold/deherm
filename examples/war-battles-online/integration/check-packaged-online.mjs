...
async function run() {
  ...
  try {
    ...
    const clientServerUrl = fallbackMode
      ? `https://localhost:${await freeLoopbackPort()}`
      : serverUrl;
    ...
    const telemetry = await waitFor(async () => {
      ...
      return value?.mode === "online" && value.state === "ready"
        ...
        ? value
        : false;
    }, { timeoutMs: 60_000, intervalMs: 100, what: `the packaged Defold game to exchange ${fallbackMode ? "WebSocket/TCP" : "QUIC"} traffic (telemetry=${JSON.stringify(lastTelemetry)}, transcript=${JSON.stringify(page.client.transcript.slice(-12))}, server=${JSON.stringify(serverLines.slice(-12))})` });

    const expectedFallbackFailures = page.client.failures.filter((failure) => fallbackMode
      && failure.kind === "log"
    ...
    const fatal = page.client.failures.filter((failure) => !failure.url?.endsWith("/favicon.ico")
      && !expectedFallbackFailures.includes(failure));
    ...
    const authoritativeHealth = await waitFor(async () => {
      let response;
      let body;
      try {
      ...
      if (!response.ok || !Number.isFinite(inputsAccepted) || inputsAccepted < minimumInputsAccepted || !markerObserved) {
        throw new Error([
    ...
    assert.equal(expectedFallbackFailures.length, fallbackMode ? 1 : 0,
    ...
    if (fallbackMode) {
      assert.ok(page.client.transcript.some((line) => line.includes("war-battles:arena-online-fallback:") && line.endsWith(":websocket-tcp")));
