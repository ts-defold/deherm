...
to host loopback only for `/healthz` and `/readyz`. The named certificate volume
is reused on restart so Chrome's pinned certificate hash remains valid. It will
