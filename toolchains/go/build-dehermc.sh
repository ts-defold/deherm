#!/usr/bin/env bash
# Cross-compile dehermc, déherm's TypeScript transform compiler.
#
...
case "$host_key" in
  ...
  *)
    echo "build-dehermc.sh: unknown host key '$host_key'" >&2
...
if [ ! -f "$ttsc_dir/go.mod" ]; then
  echo "build-dehermc.sh: the ttsc Go module is not present at $ttsc_dir" >&2
...
binary="$output_dir/dehermc$suffix"

echo "build-dehermc.sh: $host_key ($goos/$goarch) from ttsc $ttsc_version"
