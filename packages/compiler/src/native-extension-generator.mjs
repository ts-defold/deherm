...
function renderVerificationDriver(ir, routes, exactDispatch) {
  ...
  for (const [routeIndex, route] of routes.entries()) {
    ...
    definitions.push(`extern "C" ${route.result.nativeType} ${route.names.fakeCallee}(${route.parameters.map(({ type }, index) => `${type.nativeType} arg${index}`).join(",")}){if(${conditions.length ? `!(${conditions.join("&&")})` : "false"})deherm_exact_failure=${routeIndex + 1};${returnStatement}}`);
    const setups = inputs.map(renderCellSetup).join("");
    const id = `UINT32_C(${route.numericId})`;
    cases.push(`{DehermDmSdkUniversalValue arguments[${Math.max(1, route.parameters.length)}]={};DehermDmSdkUniversalValue result={};${setups}if(${exactDispatch}(${id},arguments,${route.parameters.length},&result)!=DEHERM_DMSDK_UNIVERSAL_OK)return deherm_exact_fail("dispatch",UINT32_C(${routeIndex}),${id});if(!(${renderResultCheck(route.result, output)}))return deherm_exact_fail("result",UINT32_C(${routeIndex}),${id});if(deherm_exact_failure)return deherm_exact_fail("arguments",UINT32_C(${routeIndex}),${id});}`);
