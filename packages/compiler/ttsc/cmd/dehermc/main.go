// Command dehermc is déherm's TypeScript transform compiler.
//
...
// Stamped by toolchains/go/build-dehermc.sh through -ldflags -X. These are
...
func run(args []string) int {
	...
	case "help", "--help", "-h":
		...
		return 0
	default:
		fmt.Fprintf(os.Stderr, "dehermc: unknown command %q\n", args[0])
		usage(os.Stderr)
		return 2
	}
}
...
func printVersion() int {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "dehermc: cannot encode version: %v\n", err)
		return 1
	}
...
func usage(out *os.File) {
	fmt.Fprint(out, `dehermc - déherm's TypeScript transform compiler

Usage:
  dehermc transform --tsconfig <path> [--cwd <dir>] [--plugins-json <json>]
  dehermc check     --tsconfig <path> [--cwd <dir>] [--plugins-json <json>]
  dehermc build     --tsconfig <path> --outdir <dir> [--cwd <dir>]
  dehermc serve     --tsconfig <path> [--cwd <dir>]
  dehermc version
