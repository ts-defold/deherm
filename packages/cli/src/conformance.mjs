...
const inputFiles = Object.freeze({
  scriptIr: "defold-script-api-ir.json",
  dmsdkIr: "defold-sdk-ir.json",
  scriptPatterns: "defold-script-binding-patterns.json",
...
const projectInputFiles = Object.freeze({
  scriptIr: "script-api.json",
  dmsdkIr: "dmsdk.json",
  scriptPatterns: "script-binding-patterns.json",
...
function readJson(relative, root) {
...
export async function writeConformanceHarness(plan, output, options = {}) {
  const outputDirectory = path.resolve(output);
  await mkdir(outputDirectory, { recursive: true });
  let sdkRoot = options.sdkRoot;
  if (options.sdkTemplateRoot) {
    sdkRoot = path.join(outputDirectory, "sdk");
