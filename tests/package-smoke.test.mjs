...
export async function prepareCurrentHostDehermc(root, dehermCacheHome, options = {}) {
  ...
  const tags = JSON.parse(
    await readFile(path.join(repositoryRoot, "packages", "toolchains", "release-tags.json"), "utf8"),
  );
  ...
  assert.equal(
    download && Boolean(suppliedArchive || suppliedBinary),
    false,
...
test("command-specific target parsing keeps dev endpoints separate from conformance targets", () => {
  ...
  const development = parseArguments(["dev", "--target", "http://127.0.0.1:8001", "--target", "http://127.0.0.1:8002"]);
  ...
  assert.deepEqual(
    {
      ...
      replaceDebugger: profile.replaceDebugger,
    },
...
test("packed npm artifact loads its CLI and one-shot dev compiler", async (t) => {
  ...
  const packed = JSON.parse(run("npm", ["pack", "--json", "--cache", cache, "--pack-destination", root]).stdout)[0];
  const packedFiles = new Set(packed.files.map(({ path: relative }) => relative));
  assert.equal(
    packedFiles.has("packages/generator/src/policy/generate-api-policy.mjs"),
    false,
    "repo-only policy derivation must not ship in the consumer package",
  );
  assert.equal(
    packedFiles.has("packages/compiler/src/generated/dmsdk-universal-recipes.mjs"),
    false,
  ...
  assert.equal(
    packedFiles.has("defold/defold_hermes/include/libhermesvm-config.h"),
    false,
    "the npm package leaked a target-specific Hermes config header",
  );
  assert.equal(
    packedFiles.has("scripts/assemble-typed-native-extension.mjs"),
    true,
  ...
  await assert.rejects(
    readFile(
      path.join(packageRoot, "packages", "bindings", "generated", "defold-script-real-engine-probes.json"),
      "utf8",
    ),
    (error) => error?.code === "ENOENT",
  );
  const staticFrame = await import(
    pathToFileURL(path.join(packageRoot, "packages", "compiler", "src", "dmsdk-universal-static-frame.mjs"))
  );
  const emittedStaticFrame = staticFrame.emitDmSdkUniversalStaticFrame();
  assert.equal(emittedStaticFrame.argumentCapacity, 32);
  const jsiExactRunner = await import(
    pathToFileURL(path.join(packageRoot, "packages", "compiler", "src", "dmsdk-universal-jsi-exact-runner.mjs"))
  );
  ...
  assert.equal(
    await readFile(
      path.join(
        ...
        "include",
        "defold_hermes",
        "generated_dmsdk_universal_static_frame.h",
      ),
      "utf8",
    ),
    emittedStaticFrame.header,
  );
  assert.equal(
    await readFile(
      path.join(packageRoot, "defold", "defold_hermes", "src", "generated_dmsdk_universal_static_frame.cpp"),
      "utf8",
    ),
    emittedStaticFrame.source,
  );
  assert.equal(
    await readFile(
      path.join(packageRoot, "packages", "static-hermes", "src", "generated", "dmsdk-universal.ts"),
      "utf8",
    ),
    emittedStaticFrame.staticHermes,
  );
  ...
  const store = path.join(
    ...
    "generated",
    "policy",
    shippedIndex.base.layoutVersion,
  );
  ...
  const objects = new Map(
    await Promise.all(
      Object.entries(policy.subtrees).map(async ([namespace, digest]) => [
        namespace,
        {
          digest,
  ...
  const { materializePolicySurface } = await import(
    pathToFileURL(path.join(packageRoot, "packages", "compiler", "src", "policy-surface-materializer.mjs"))
  );
  ...
  await assertMaterializedFiles(
    path.join(packedSurfaceRoot, "sdk", "generated"),
    materialized.descriptor.sdk,
    entry.defoldRevision,
  );
  await assertMaterializedFiles(
    path.join(packedSurfaceRoot, "repository"),
    materialized.descriptor.outputs,
    entry.defoldRevision,
  );
  ...
  await readFile(
    path.join(
      ...
      "defold_hermes",
      "src",
      "generated_scalar_lua_descriptors.cpp",
    ),
    "utf8",
  );
  ...
  const packedRecipe = packedCatalogDocument.recipes.find(
    ({ symbol, declarationKind }) => symbol === "dmMath::Clamp" && declarationKind === "function-template",
  );
  ...
  const packedMaterialization = run(
    process.execPath,
    [
  ...
  const packedVerificationReport = JSON.parse(await readFile(packedProvider.replace(/\.cpp$/, ".verify.json"), "utf8"));
  assert.equal(packedVerificationReport.vectors[0].invocation.kind, "function-template-specialization");
  const packedJsiVerification = await readFile(packedProvider.replace(/\.cpp$/, ".verify.jsi.cpp"), "utf8");
  ...
  const packedJsiReport = JSON.parse(await readFile(packedProvider.replace(/\.cpp$/, ".verify.jsi.json"), "utf8"));
  ...
  await writeFile(
    path.join(sdkInclude, "dmsdk", "dlib", "math.h"),
    [
      "#pragma once",
      "namespace dmMath {",
  ...
  await writeFile(
    path.join(extensionProject, "game.project"),
    [
      "[project]",
  ...
  await writeFile(
    path.join(extensionProject, "packed", "include", "packed.h"),
    ["#include <stdint.h>", "uint32_t packed_add(uint32_t left, uint32_t right);", ""].join("\n"),
  );
  const packedProjectGeneration = run(
    process.execPath,
    [path.join(packageRoot, "bin", "deherm.mjs"), "generate", "--project", extensionProject, "--json", "--force"],
  ...
  const packedProjectIndex = JSON.parse(
    await readFile(path.join(extensionProject, ".deherm", "generated", "native-extensions", "index.json"), "utf8"),
  );
  ...
  await writeFile(
    path.join(consumer, "package-api.ts"),
    [
  ...
  await driveLanguageServer(
    process.execPath,
    [path.join(packageRoot, "bin", "deherm-language-server.mjs"), "--project", project],
  ...
  const verified = run(
    process.execPath,
    [path.join(packageRoot, "bin", "deherm.mjs"), "verify-generated", "--project", project, "--json"],
    { cwd: project, env: { ...process.env, DEHERM_CACHE_HOME: dehermCacheHome } },
  );
  ...
  const checked = run(
    process.execPath,
    [path.join(packageRoot, "bin", "deherm.mjs"), "typecheck", "--project", project, "--json"],
    { cwd: project, env: { ...process.env, DEHERM_CACHE_HOME: dehermCacheHome } },
  );
  assert.equal(JSON.parse(checked.stdout).passed, true);

  const releaseChecked = run(
    process.execPath,
    [path.join(packageRoot, "bin", "deherm.mjs"), "typecheck", "--release", "--project", project, "--json"],
    {
      cwd: project,
      env: {
        ...compilerEnvironment,
  ...
  const packedReleaseUsage = JSON.parse(
  ...
  const development = run(
    ...
    [
      ...
      "dev",
      "--entry",
      path.relative(root, path.join(project, "src", "main.script.ts")),
      "--out-dir",
      "generated-sdk",
      "--once",
      "--headless",
