...
test("opaque records have exact source-pinned ownership blockers", async () => { execFileSync(process.execPath, ["scripts/generate-script-opaque-record-blockers.mjs", "--check"], { cwd: root, stdio: "pipe" }); const report = JSON.parse(await readFile(new URL("bindings/generated/defold-script-opaque-record-blockers.json", root), "utf8")); assert.equal(report.routeCount, 2); assert.equal(report.candidateCount, 0); assert.match(report.coverageClaim, /command queue/); assert.deepEqual(report.routes.map(({ id }) => id).toSorted(), ["script:render.dispatch_compute", "script:render.draw"]); });
