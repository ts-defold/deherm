...
test("opaque blocker generator reports stale evidence and rejects incomplete coverage", async () => { const inputs = await loadInputs(); const stale = new Map(inputs.sourceTexts); const [path, text] = stale.entries().next().value; stale.set(path, `${text}\n`); assert.doesNotThrow(() => generate({ ...inputs, sourceTexts: stale })); const incomplete = JSON.parse(inputs.policyText); incomplete.routes.pop(); assert.throws(() => generate({ ...inputs, policyText: JSON.stringify(incomplete) }), /frontier coverage expected 2, found 1|does not cover the complete frontier/); });
