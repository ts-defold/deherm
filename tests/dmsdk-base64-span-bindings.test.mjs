...
test("base64-span generator rejects mixed provenance and withdraws only drifted optimization evidence", async () => {
  ...
  try {
    ...
    run(process.execPath, ["scripts/generate-dmsdk-base64-span-bindings.mjs", "--policy", policyPath, "--out-root", join(output, "out")]);
    const report = JSON.parse(await readFile(join(output, "out", "packages/bindings/generated/defold-dmsdk-base64-span-bindings.json"), "utf8"));
    assert.equal(report.coverage.emitted, 1);
    assert.equal(report.coverage.policyBlocked, 1);
    assert.equal(report.declarations.find(({ emitted }) => emitted === false)?.blocker, "base64-span-evidence-withdrawn");
  } finally { await rm(output, { recursive: true, force: true }); }
});
...
test("base64 C ABI links, rejects unsafe input, and observes zero warmed C++ operator new calls", async (context) => {
  ...
  try {
    const cObject = join(output, "header.o"); run(cCompiler, ["-std=c11", "-Wall", "-Wextra", "-Werror", "-pedantic", `-I${join(repositoryRoot, "defold/defold_hermes/include")}`, "-c", "native/dmsdk_base64_span_c_header_test.c", "-o", cObject]);
    const libraries = [join(sdkRoot, "lib/arm64-macos/libdlib.a"), "-framework", "Security", "-framework", "CoreFoundation", "-framework", "Foundation"];
    const cExecutable = join(output, "c-abi"); run(compiler, ["-std=c++17", ...includeArgs(), "defold/defold_hermes/src/generated_dmsdk_base64_span_crypt.cpp", cObject, ...libraries, "-o", cExecutable]); run(cExecutable, []);
    const executable = join(output, "host"); run(compiler, ["-std=c++17", "-Wall", "-Wextra", "-Werror", "-pedantic", ...includeArgs(), "defold/defold_hermes/src/generated_dmsdk_base64_span_crypt.cpp", "defold/defold_hermes/src/generated_dmsdk_base64_span_runtime.cpp", "native/dmsdk_base64_span_host_test.cpp", ...libraries, "-o", executable]); run(executable, []);
  } finally { await rm(output, { recursive: true, force: true }); }
});

test("base64 generated C++ has no heap ownership primitive", async () => { const report = JSON.parse(await readFile(reportPath, "utf8")); for (const artifact of report.artifacts.filter((path) => path.endsWith(".cpp"))) assert.doesNotMatch(await readFile(join(repositoryRoot, artifact), "utf8"), /\b(?:new|delete|malloc|calloc|realloc|free)\b/, artifact); });
