...
struct Backend { uint32_t calls = 0; bool badField = false; std::array<ScriptTableEntry, 6> entries{}; };
records::DispatchStatus invoke(void* raw, const records::Operation& operation, const records::Codec*, const records::Field* fields, ScriptCallFrame* frame, char*, size_t) noexcept {
  ...
  for (uint32_t index = 0; index < operation.fieldCount; ++index) { backend->entries[index].key = string(fields[operation.fieldOffset + index].name); backend->entries[index].value = index == 0 && fields[operation.fieldOffset + index].codec == records::Codec::kString ? string("1.2.3") : integer(static_cast<int>(index + 1)); }
  if (backend->badField) backend->entries[operation.fieldCount - 1].key = string("unexpected");
  frame->results[0] = {}; frame->results[0].tag = ScriptValueTag::kTable; frame->results[0].data = backend->entries.data(); frame->results[0].length = operation.fieldCount; frame->resultCount = 1;
  return records::DispatchStatus::kSuccess;
}
struct Storage { std::array<ScriptValue, 1> args{}; std::array<ScriptValue, 1> results{}; std::array<char, 128> error{}; ScriptCallFrame frame{}; Storage() { frame.stableId = UINT32_C(0xea93e5f3); frame.arguments = args.data(); frame.results = results.data(); frame.resultCapacity = results.size(); } };
}  // namespace
