...
int MockRoute(lua_State* state) {
  ...
  if(route.context==handle::Context::kGameObjectInstance)expect(hasExpectedInstance(state),"game-object instance was not installed");
...
void installRoutes(lua_State* state,const handle::RuntimeProfile& profile) {
  for(size_t index=0;index<handle::kRouteCount;++index){const auto& route=handle::routes()[index];if(route.nativeDynamicHermes!=handle::Disposition::kCapturedLuaRouterHarnessProvenJsiUnverified||!handle::routeAvailableInProfile(route,profile))continue;
...
bool callRoute(scalar::ScriptAdapter& adapter,const handle::Route& route,
  ...
  const bool ok=adapter.dispatch(&frame);if(ok&&frame.resultCount&&frame.results[0].handleKind==ScriptHandleKind::kLuaSemanticHandle){auto api=adapter.api();api.releaseHandle(api.context,frame.results[0].handleKind,frame.results[0].length,frame.results[0].payload);}return ok;
}
...
void runProfile(const handle::RuntimeProfile& runtimeProfile,bool fullBehavior,std::array<bool,handle::kRouteCount>& unionCovered){lua_State* state=luaL_newstate();expect(state!=nullptr,"Lua state creation failed");installRoutes(state,runtimeProfile);const auto detected=detectProfile(state,handle::RuntimeProfileDetectionStatus::kMatched,"exact profile surface was not detected");expect(detected.profile==&runtimeProfile,"exact profile surface selected the wrong profile");expect(detected.observedPresent==runtimeProfile.adapterExecutableRouteCount,"detected profile route census drifted");int instance=0;gExpectedInstance=&instance;lua_pushlightuserdata(state,gExpectedInstance);SetInstance(state);gCalls=0;gReenter=false;gNestedFrame={};
  ...
  auto terminalFor=[&](handle::SemanticHandleKind kind)->const handle::Route*{for(size_t i=0;i<handle::kRouteCount;++i){const auto& route=handle::routes()[i];if(route.nativeDynamicHermes==handle::Disposition::kCapturedLuaRouterHarnessProvenJsiUnverified&&handle::routeAvailableInProfile(route,runtimeProfile)&&route.operationClass==handle::OperationClass::kTerminal&&firstKind(route)==kind)return &route;}return nullptr;};
  ...
  const handle::Route* blocked=nullptr;for(size_t i=0;i<handle::kRouteCount;++i)if(handle::routes()[i].nativeDynamicHermes!=handle::Disposition::kCapturedLuaRouterHarnessProvenJsiUnverified){blocked=&handle::routes()[i];break;}expect(blocked&&!callRoute(adapter,*blocked,handles),"blocked context route executed");
...
void runProfileDetectionNegatives(){const auto* profile=handle::findRuntimeProfile("default-legacy-bullet");expect(profile,"profile detection fixture missing");const handle::Route* present=nullptr;const handle::Route* absent=nullptr;for(size_t i=0;i<handle::kRouteCount;++i){const auto& route=handle::routes()[i];if(route.nativeDynamicHermes!=handle::Disposition::kCapturedLuaRouterHarnessProvenJsiUnverified)continue;if(handle::routeAvailableInProfile(route,*profile)&&!present)present=&route;if(!handle::routeAvailableInProfile(route,*profile)&&!absent)absent=&route;}expect(present&&absent,"profile detection mutation routes missing");
