...
void installRoutes(lua_State* state,const handle::RuntimeProfile& profile) {
  // Profile detection probes the Lua registration surface, which includes
  // registered functions whose result handles are not capturable by this
  // adapter profile (for example Box2D world lightuserdata). Dispatch below
  // still uses routeAvailableInProfile and therefore retains the executable
  // adapter/runtime distinction.
  for(size_t index=0;index<handle::kRouteCount;++index){const auto& route=handle::routes()[index];if(!route.nativeAdapterHarness||!(route.registrationProfileMask&profile.mask))continue;
...
uint16_t registeredHarnessRouteCount(const handle::RuntimeProfile& profile) {
  uint16_t count=0;
  for(size_t index=0;index<handle::kRouteCount;++index){const auto& route=handle::routes()[index];if(route.nativeAdapterHarness&&(route.registrationProfileMask&profile.mask))++count;}
  return count;
}
...
void runProfileDetectionNegatives(){const auto* profile=handle::findRuntimeProfile("default-legacy-bullet");expect(profile,"profile detection fixture missing");const handle::Route* present=nullptr;const handle::Route* absent=nullptr;for(size_t i=0;i<handle::kRouteCount;++i){const auto& route=handle::routes()[i];if(!route.nativeAdapterHarness)continue;if((route.registrationProfileMask&profile->mask)&&handle::routeAvailableInProfile(route,*profile)&&!present)present=&route;if(!(route.registrationProfileMask&profile->mask)&&!absent)absent=&route;}expect(present&&absent,"profile detection mutation routes missing");
  {lua_State* state=luaL_newstate();installRoutes(state,*profile);removeRoute(state,*present);installRoute(state,*absent);const auto result=detectProfile(state,handle::RuntimeProfileDetectionStatus::kNoMatch,"same-count wrong vector matched by count");expect(result.observedPresent==registeredHarnessRouteCount(*profile),"same-count fixture route census drifted");lua_close(state);}
